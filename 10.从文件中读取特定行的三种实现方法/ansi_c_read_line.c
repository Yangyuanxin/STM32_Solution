/*
这个版本不依赖于POSIX API，如`mmap，`而是坚持ANSI C功能。
此版本也适用于不可搜索的文件，因此可以通过管道馈送。
它执行有限但充分的错误检查。也就是说`，get_nth_line`在所有失败时都返回 NULL，
并且调用方可以通过在输入文件上调用`feof（）`和`ferror（）`来区分 EOF、文件读取错误和内存不足。 
*/
#include <stdio.h>
#include <stdlib.h>
