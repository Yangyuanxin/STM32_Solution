...
const char * record_fromfile_get(void *datafile, int *record_len)
{
  ...
  rec_end = (char *)hbm_patternmatch((unsigned char *)rec_beg, in_argp->buf_data_rem, (unsigned char *)rec_end_pat, rec_end_pat_len, 0,codetype);
...
int record_parse(void *rec_mode,const char *record, int record_len)
{
  ...
  for (i = 0;i < recmode->field_num;i++)
  {
    mwmAddPatternEx(pv, (unsigned char *)(recmode->field[i].begtag), strlen(recmode->field[i].begtag), 0, 0, 0, (void *)i, 3000);
  }
  mwmPrepPatterns(pv);
  mwmSearch(pv, (unsigned char *)record, rec_len, match, &mat_arg);
  mwmFree(pv);
...
static int match(void *id, int index, void *data)
{
  ...
  pend = (char *)hbm_patternmatch((unsigned char *)pbeg, mat_arg_p->rec_end - pbeg, (unsigned char *)mat_arg_p->recmode->field[fid].endtag, strlen(mat_arg_p->recmode->field[fid].endtag), 0,mat_arg_p->recmode->codetype);
  if (!pend)
  {
    return 0;
  }
