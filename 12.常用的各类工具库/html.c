...
int html_filterjavascript(const char *input,int inlen,char *output,int outlen,int codetype)
{
  ...
  // <script ...> ... </script ...>
  while ((pb = (char *)hbm_patternmatch((unsigned char *)txt,txtend-txt,(unsigned char *)"<script",7,1,codetype)) != NULL)
  {
...
#define COPY_TO_BUFFER(B,BLEN,BSIZE,S,SLEN) if((SLEN) < ((BSIZE)-(BLEN))) {  \
...
// only support utf-8
static int highlight_pos_replace(char *text,int text_len,
                                     char *kw[],
                                     highlight_t *hl,
                                     char *first_pos[],
                                     int kw_num,
                                     hlposarg_t *arg,
                                     char *outputbuffer, int bufsize)
{
  ...
  restart:
   while (po != po_end)
   {
     ...
     clen = UTF8CHLEN(*((unsigned char *)po));
